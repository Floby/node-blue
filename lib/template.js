...
Template.prototype._newData = function _newData(logme) {
    var filenames = this._buffers.map(function(e) {
        return e.template.filename;
    });
    //console.log('in %s', this.filename ,filenames);
    
    // buffer data if no listener is there
    // to consume

    //console.log('buffer %s is finished %s', this._buffers[0] && this._buffers[0].template.filename, this._buffers[0] && this._buffers[0].finished)
    // flush all finished buffers
    while(this._buffers[0] && this._buffers[0].finished) {
        //console.log('flushing from %s', this._buffers[0].template.filename);
        this._write(this._buffers[0].getData());
        var shifted = this._buffers.shift();
    ...
    if(this._buffers[0] && !this._buffers[0].isEmpty()) {
        //console.log('semiflushing from %s', this._buffers[0].template.filename);
        this._write(this._buffers[0].getData());
